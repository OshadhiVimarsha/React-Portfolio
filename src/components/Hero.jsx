...
const Hero = () => {
  ...
  return (
    <section
      className={`overflow-hidden relative transition-colors duration-500 ${darkMode ? "dark" : ""}`}
      id="home"
    >
      ...
      <div className="min-h-screen flex md:flex-row flex-col-reverse md:items-center justify-center items-center px-6 relative">
        {/* Left Column */}
        <div className="pb-16 pt-10 text-center" data-aos="fade-down">
          <h3 className="text-base md:text-lg text-gray-600 dark:text-gray-300">{hero.subtitle}</h3>
          <h1 className="py-2 md:py-4 text-3xl md:text-5xl font-bold text-cyan-900 dark:text-white">
            {hero.name}
          </h1>
          <h2 className="text-cyan-600 dark:text-cyan-400 text-xl md:text-2xl">{hero.title}</h2>
          ...
          <div className="flex justify-center mt-6 py-2">
            <a
              href={hero.link}
              className="relative overflow-hidden px-6 py-3 font-medium text-white rounded-xl bg-cyan-500 group hover:bg-cyan-700 transition-colors duration-300"
            >
              <span className="relative z-10 text-white">{hero.btnText}</span>
              <span className="absolute inset-0 bg-gradient-to-r from-cyan-700 via-cyan-500 to-cyan-600 transform -translate-x-full group-hover:translate-x-0 transition-transform duration-500 rounded-xl"></span>
